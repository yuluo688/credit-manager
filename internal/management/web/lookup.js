...
(() => {
  ...
  function t(source) { return locale==='zh-CN' ? source : ((COPY[source] && COPY[source][locale]) || source); }
  function translateElement(element) { if (!element || element.closest('[data-no-i18n]')) return; ['title','placeholder','aria-label'].forEach(name=>{ if (!element.hasAttribute(name)) return; let values=attributeSources.get(element); if (!values) { values={}; attributeSources.set(element, values); } if (!(name in values)) values[name]=element.getAttribute(name); element.setAttribute(name, t(values[name])); }); const textNodes=[...element.childNodes].filter(node=>node.nodeType===Node.TEXT_NODE && node.nodeValue.trim()); if (!textNodes.length) return; let values=textSources.get(element); if (!values) { values=new Map(); textSources.set(element, values); } textNodes.forEach(node=>{ if (!values.has(node)) values.set(node, node.nodeValue); const source=values.get(node), leading=source.match(/^\s*/)[0], trailing=source.match(/\s*$/)[0]; node.nodeValue=leading+t(source.trim())+trailing; }); }
