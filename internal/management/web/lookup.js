...
(() => {
  ...
  function table(headers, rows) { return rows.length?'<table><thead><tr>'+headers.map(h=>'<th>'+esc(t(h))+'</th>').join('')+'</tr></thead><tbody>'+rows.join('')+'</tbody></table>':'<div class="empty"><span class="empty-icon" aria-hidden="true">'+uiIcon('list')+'</span><span>'+esc(t('当前筛选条件下暂无数据'))+'</span></div>'; }
  ...
  function chart(id, empty, option) { const target=$(id); destroyChart(id); if(!option||!window.echarts) { target.className='chart chart-empty'; target.innerHTML='<span class="empty-icon" aria-hidden="true">'+uiIcon('chart')+'</span><span>'+(window.echarts?esc(empty):esc(t('图表组件加载失败')))+'</span>'; return; } target.className='chart'; target.textContent=''; const instance=echarts.init(target,null,{renderer:'svg'}); instance.setOption(option); charts[id]=instance; }
