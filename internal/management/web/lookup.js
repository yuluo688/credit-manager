...
(() => {
  ...
  function chart(id, empty, option, icon) { const target=$(id); destroyChart(id); if(!option||!window.echarts) { target.className='chart chart-empty'; target.innerHTML='<span class="empty-icon" aria-hidden="true">'+uiIcon(icon||'chart')+'</span><span>'+(window.echarts?esc(empty):esc(t('图表组件加载失败')))+'</span>'; return; } target.className='chart'; target.textContent=''; const instance=echarts.init(target,null,{renderer:'svg'}); instance.setOption(option); charts[id]=instance; }
